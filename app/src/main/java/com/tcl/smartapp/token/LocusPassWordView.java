...
public class LocusPassWordView extends View {
        ...
        for (int i = 0; i < mPoints.length; i++) {
            for (int j = 0; j < mPoints[i].length; j++) {
                ...
                if (p.state == Point.STATE_CHECK) {
                    ...
                    canvas.drawCircle(p.x, p.y, mCircleRadius, selectedPaint);
                    //canvas.drawOval(p.x, p.y, p.x + 25, p.y + 25, selectedPaint);
                } else if (p.state == Point.STATE_CHECK_ERROR) {
                    inErrorState = true;
                    errorPaint.setColor(color);
                    //canvas.drawCircle(p.x, p.y, dotRadius, errorPaint);
                    errorPaint.setColor(color);
                    canvas.drawCircle(p.x, p.y, mCircleRadius, errorPaint);
                    //canvas.drawOval(p.x, p.y, p.x + 25, p.y + 25, errorPaint);
                } else {
                    normalPaint.setColor(color);
                    //canvas.drawCircle(p.x, p.y, dotRadius, normalPaint);
                    normalPaint.setColor(color);
                    canvas.drawCircle(p.x, p.y, mCircleRadius, normalPaint);
                    //canvas.drawOval(p.x, p.y, p.x + 25, p.y + 25, selectedPaint);
                }
            }
        }
    ...
    private void drawLine(Point start, Point end, Canvas canvas, Paint paint) {
        ...
        float rx = 0;//(float) ((end.x - start.x) * 1 / 4 / d);
        float ry = 0;//(float) ((end.y - start.y) * 1 / 4 / d);
        canvas.drawLine(start.x+2, start.y+2, end.x+2, end.y +2, paint);
    }
    ...
    private void drawArrow(Canvas canvas, Paint paint, Point start, Point end, float arrowHeight, int angle) {
        ...
        float h = (float) (d - arrowHeight - 1 * 1.1);
    ...
    private void initCache() {
        ...
        int leftPadding = 15;
        float dotPadding = width / 3 - leftPadding -20;
    ...
    private void initPaints() {
        mCircleRadius = SmartApplication.mContext.getResources().getDimensionPixelSize(R.dimen.draw_password_circle_radius);
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        if (isFinish) {
            if (this.sPoints.size() == 1) {
                ...
                error();
                clearPassword();
                //Toast.makeText(this.getContext(), R.string.pls_try_again,
